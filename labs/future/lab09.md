...
You should replace the `-ciphertype` with a specific cipher type, such as `-aes-128-cbc`, `-aes-128-cfb`, etc.
In this task, you should try at least 3 different ciphers.
Please note any observations.

#### Task 3: Encryption Modes --- ECB vs. CBC

In this task, you must encrypt a picture so that people without the encryption key cannot know what is in the picture.

###### Task 3.1

1. Please download and encrypt the file [pic_original.bmp](https://seedsecuritylabs.org/Labs_16.04/Crypto/Crypto_Encryption/files/pic_original.bmp), which contains a simple picture.
   You should produce two encrypted versions of the file: one using the **ECB** (Electronic Code Book) and another using the **CBC** (Cipher Block Chaining) mode.

   **Tip.** You can get the picture and store it in the current directory easily with `wget`:

   ```bash
   $ wget https://seedsecuritylabs.org/Labs_16.04/Crypto/Crypto_Encryption/files/pic_original.bmp
   ```

2. Let us treat the encrypted picture as a picture, and use a picture viewing application to display it.
...
3. Display the encrypted picture using a picture viewing program (there is an image viewer program called `eog` on the SEED VM).
...
_**NOTE:** Be careful with your selection.
I still recommend using a `.bmp` file, unless you are feeling adventerous!
You will (likely) need to do some independent research to determine how the header works for a different image type if you use a different image type for this part._
...
Let us create three files, which contain 5 bytes, 10 bytes, and 16 bytes, respectively.
...
It should be noted that padding data may not be printable, so you need to use a hex tool to display the content (e.g., `xxd`, `hexdump`).
For example:
...
Repeat **Task 4.1** for each of the following modes of operation (use the `aes` cipher): **ECB**, **CFB**, **OFB**.
...
3. Intentionally corrupt a single bit of the 55th byte in the encrypted file. (You can achieve this corruption using, e.g., the `bless` or `ghex` hex editor.)
4. Decrypt the corrupted ciphertext file using the correct key and IV.
5. Answer this question: _For this encryption mode, was your hypothesis from Part I correct? Explain._

Again, _**you must repeat Task 5.2 for each encryption mode:**_ ECB, CBC, CFB, and OFB.
...
A basic requirement for IV is _**uniqueness**_, which means that no IV may be reused under the same key.
...
_**Hint:**_
_You may find it useful to have a tool that can help you XOR some values in this task._
_You are free to use whatever XOR tool you want._
_I wrote a simple python script for my solution._
_Others have used an online [XOR calculator](https://xor.pw)._
_Feel free to do whatever makes the most sense for you._
...
Another important requirement for IV is that IVs need to be _**unpredictable**_ for many schemes,
...
###### Final Thoughts on Initialization Vectors
...
For those that are interested, there is another [SEED lab (Pseudo Random Number Generation Lab)](https://seedsecuritylabs.org/Labs_16.04/Crypto/Crypto_Random_Number/),
which covers details on how to generate cryptographically strong pseudo random numbers.

#### Task 7 (Extra Credit): Programming using the Crypto Library

In this task, you are given a plaintext and a ciphertext, and your goal is to write a Python program or C program (your choice, though I recommend Python)
