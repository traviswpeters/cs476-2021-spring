...
Using the attacks, students should be able to create two different programs that share the same MD5 hash but have completely different behavior.  
...
<center class="mt-4 mb-4"><img src="./figs/hash-fig1.png" width="700" alt="Overview of how the md5collgen program works."></center>
...
_**NOTE:** The prefix may be set to whatever content you want._
...
Regarding the resulting collision from Task 1.3, is the data (128 bytes) generated by `md5collgen` completely different for the two output files?
Please explain your answer and also identify any/all bytes that are different.
...
MD5 is a complicated algorithm, but for our purposes we only need to understand how the algorithm works at a high level.
...
The core of the MD5 algorithm is a **_compression function_**,
...
<center class="mt-4 mb-4"><img src="./figs/hash-fig2.png" width="900" alt="Overview of how the md5 algorithm works."></center>
...
_**Your task**_ is to design an experiment to demonstrates that this property holds for MD5.
...
Then you can use a hex editor to modify the content of the `xyz` array directly in the binary file.
...
For example, the following code snippet shows how to fill the array with `0x41`, which is the ASCII value for the letter A.
It should not be difficult to locate 200 A's in the binary.
(The [print_array.c](https://github.com/traviswpeters/cs476-code/blob/master/07_hash/print_array.c) program
in the course repo has already taken this approach if you want to use that as your starting point.)
...
See the figure below for an illustration of how the file is divided.

<!-- <center class="mt-4 mb-4"><img src="../assets/lab10_prefix_data_suffix.png" width="600" alt="..."></center> -->
<center class="mt-4 mb-4"><img src="./figs/hash-fig3.png" width="800" alt="Break the executable file into 3 pieces."></center>
...
#### Task 3 Tools
You can use any hex editor (e.g., `ghex`, `bless`) to view the binary executable file and find the location of the array.
...
With these commands, we can divide a binary file into pieces from any location.
If we need to concatenate pieces together, we can use the `cat` command.
...
In the previous task, we successfully created two programs that have the same MD5 hash, but their behavior is only superficially different.
...
they will have no concerns when downloading and executing your (malicious) program.

_**The objective of this task**_ is to launch the attack described in the motivation above.
...
The figure below illustrates what the two versions of the program look like.

<!-- <center class="mt-4 mb-4"><img src="../assets/lab10_collision.png" width="600" alt=""></center> -->
<center class="mt-4 mb-4"><img src="./figs/hash-fig4.png" width="800" alt="An approach to generate two hash-colliding programs with different behavior."></center>

From this figure we know that these two binary files have the same MD5 hash value,
...
If you wish to use this approach, the relevant starter code can be found in
[benign_evil.c](https://github.com/traviswpeters/cs476-code/blob/master/07_hash/benign_evil.c) in the course repo.
